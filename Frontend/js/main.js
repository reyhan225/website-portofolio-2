...
function showFormAlert(el, msg, type) {
  ...
  setTimeout(() => { el.className = 'form-alert'; }, 5000);
}
...
const terminalCommands = {
  ...
  skills: () => [
    ...
    { cls: 'term-output', text: '  Frontend  : HTML5, CSS3, JS, React, Vue' },
    { cls: 'term-output', text: '  Backend   : Node.js, Python, Laravel, Flask' },
...
function initScrollAnimation() {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    document.querySelectorAll('.skill-category, .project-card, .detail-card, .case-block').forEach(el => {
      el.style.opacity = '1';
